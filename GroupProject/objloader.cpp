...
bool loadOBJ2(
	...
	while (1){
		...
		if (strcmp(lineHeader, "v") == 0){
			...
			std::string vertex1, vertex2, vertex3;
			unsigned int vertexIndex[3], colorIndex[3], normalIndex[3];
			int matches = fscanf(file, "%d/%d/%d %d/%d/%d %d/%d/%d\n", &vertexIndex[0], &colorIndex[0], &normalIndex[0], &vertexIndex[1], &colorIndex[1], &normalIndex[1], &vertexIndex[2], &colorIndex[2], &normalIndex[2]);
			if (matches != 9){
				printf("File can't be read by our simple parser :-( Try exporting with other options\n");
				return false;
			}
	...
	for (unsigned int i = 0; i<vertexIndices.size(); i++){
		...
		glm::vec3 color = temp_color[colorIndex];
